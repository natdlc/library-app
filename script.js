const Library = (() => {
	...
	const createBookCard = (bookDetails) => {
		...
		if (selectedOpt === "complete")
			completeOpt.setAttribute("selected", "");
		else if (selectedOpt === "in progress")
			inProgressOpt.setAttribute("selected", "");
		...
		readSelection.addEventListener("click", (e) => {
			library[title.innerText].readStatus = e.path[0].value;
		});
	};
	...
	const createNewBookModal = () => {
		...
		titleInp.addEventListener("input", () => {
			console.log(titleInp.validity);
			if (titleInp.validity.valueMissing) {
				titleInp.setCustomValidity("Field cannot be empty");
			} else {
		...
		authorInp.addEventListener("input", () => {
			if (authorInp.validity.valueMissing) {
				authorInp.setCustomValidity("Field cannot be empty");
			} else {
		...
		submit.addEventListener("click", () => {
			if (
				titleInp.validity.valueMissing ||
				authorInp.validity.valueMissing ||
				pagesInp.validity.valueMissing
			)
				return;
			let bookInfo = [
				titleInp.value,
